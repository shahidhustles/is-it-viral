    ...
    <AuthShell>
      {isVerifying ? (
        <form className="space-y-6" onSubmit={verifyEmailCode}>
          <AuthHeading title="Check your inbox" description={`We sent a six-digit verification code to ${email}.`} />
          ...
          <Button className="w-full" disabled={isSubmitting} size="lg" type="submit">{isSubmitting ? "Verifying…" : "Verify and continue"}</Button>
          <button className="w-full text-sm font-medium underline underline-offset-4" disabled={isSubmitting} onClick={() => void signUp.verifications.sendEmailCode()} type="button">Send a new code</button>
        ...
        <form className="space-y-6" onSubmit={requestEmailCode}>
          <AuthHeading title="Give your reel a first audience." description="Create your account and start shaping the people you want to reach." />
          ...
          <AuthDivider />
          <AuthError message={errorMessage ?? errors.fields.emailAddress?.message} />
          <div className="space-y-2">
